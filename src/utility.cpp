...
std::vector<std::string_view> split(std::string_view s, char delimiter, int token_cnt) {
...
uint32_t hexToUint32(const std::string_view hex_str) {
  uint32_t value;
  auto [ptr, ec] = std::from_chars(hex_str.data(),
                                   hex_str.data() + hex_str.size(), value, 16);
  if (ec != std::errc())
    return 0;
  return value; // 호스트 바이트 오더 반환
}

// 주어진 네트워크 인터페이스 이름(예: "ens160", "eth0")의 IPv4 주소를 반환
...
void printTerminalQRCode(const std::string &name, const std::string &ip,
