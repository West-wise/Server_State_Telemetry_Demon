...
SystemReader::~SystemReader() { stop(); }
...
void SystemReader::parseProcStat(SystemStats &out) {
  ...
  if (std::getline(file, line)) {
    if (line.substr(0, 3) == "cpu") {
      ...
      iss >> header >> user >> nice >> system >> idle >> iowait >> irq >>
          softirq >> steal;
...
bool SystemReader::parseNetDevInfo(NetCounter &out) {
  ...
  while (std::getline(file, line)) {
    ...
    std::string iface_name =
        std::string(SST::Utils::String::trim(line.substr(0, pos)));
    if (iface_name == "lo")
      continue;
    ...
    if (!(iss >> rx_bytes >> rx_packets >> rx_errs >> rx_drop >> rx_fifo >>
          rx_frame >> rx_comp >> rx_mcast >> tx_bytes >> tx_packets >>
          tx_errs >> tx_drop >> tx_fifo >> tx_colls >> tx_carrier >> tx_comp)) {
      continue;
    }
...
void SystemReader::getNetDevInfo(SystemStats &out) {
  ...
  out.net_rx_bytes = {static_cast<uint32_t>(d_rx_bytes / sec),
                      static_cast<uint32_t>(d_rx_pkts / sec),
                      static_cast<uint32_t>(d_rx_errs / sec),
                      static_cast<uint32_t>(d_rx_drop / sec)};

  out.net_tx_bytes = {static_cast<uint32_t>(d_tx_bytes / sec),
...
void SystemReader::networkConnectedClients(SystemStats &stats) {
  ...
  for (const auto &path : source) {
    ...
    while (std::getline(file, line)) {
      ...
      if (!(iss >> sl >> local >> remote >> state))
        continue;
      if (state == "01") {
        auto tokens = SST::Utils::String::split(remote, ':', 2);
        if (tokens.size() < 2)
          continue;
        ip_set.insert(std::string(tokens[0]));
      }
    }

    stats.net_user_count = ip_set.size();
  }
}
