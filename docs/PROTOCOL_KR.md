...
Noise 핸드셰이크와 연결 요청을 완료한 클라이언트에게 서버가 주기적으로 상태 정보를 전송하는 **Push 모델 (Streaming)**을 따릅니다.
...
1. **서버 신원 검증**: Noise_XX 패턴. 클라이언트는 QR 코드의 `pub_key`로 서버 신원을 검증합니다. 현재 서버는 클라이언트의 별도 신원을 검증하지 않고 핸드셰이크와 연결 요청을 완료한 클라이언트를 수락합니다.
