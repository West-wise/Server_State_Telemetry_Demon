...
class Logger {
public: // 모든 멤버 변수를 static으로 선언하여 싱글톤 구현
  // Async Logger 초기화
  static bool init(std::string_view log_path) {
    std::lock_guard<std::mutex> lock(
        mutex_); // 락 획득 및 멀티스레드 환경에서 안전하게 인스턴스 생성
    if (instance_)
      return true; // 이미 초기화됨
    ...
    } catch (const std::exception &e) { // 기존의 리다이렉트 방식을 사용X, 로깅
                                        // 스레드를 사용해서 비동기적 처리 채택
